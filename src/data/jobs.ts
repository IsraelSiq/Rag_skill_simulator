...
export interface JobMeta {
  ...
  tier: 1 | 2 | 3 | 4
  parent?: JobId
  // pontos de skill totais disponíveis nessa classe
  skillPoints: number
...
export const JOBS: JobMeta[] = [
  ...
  { id: 'windhawk',        name: 'Windhawk',         tier: 4, parent: 'ranger',         skillPoints: 250, icon: '🦆' },
...
// Monta a chain completa de jobs (ex: rune-knight → lord-knight → knight → swordman → novice)
export function getJobChain(jobId: JobId): JobId[] {
  ...
  // adiciona novice no final se não estiver lá
  if (!chain.includes('novice')) chain.push('novice')
  return chain
}

// Agrupa jobs por tier para o seletor
...
// Retorna os filhos diretos de um job
